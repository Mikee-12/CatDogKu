...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_staff'])) {
    ...
    if ($nama_staff !== '' && $jabatan !== '') {
        $spec_val = $id_specialization > 0 ? $id_specialization : 'NULL';
        mysqli_query($conn,
            "INSERT INTO staffs (nama_staff, id_specialization, jabatan, no_telepon, is_active)
             VALUES ('$nama_staff', $spec_val, '$jabatan', '$no_telepon', $is_active)"
        );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_staff'])) {
    ...
    if ($id_staff > 0 && $nama_staff !== '' && $jabatan !== '') {
        $spec_val = $id_specialization > 0 ? $id_specialization : 'NULL';
        mysqli_query($conn,
            "UPDATE staffs
             SET nama_staff='$nama_staff', id_specialization=$spec_val,
                 jabatan='$jabatan', no_telepon='$no_telepon', is_active=$is_active
...
.btn-edit-act { background: #fef9e7; color: #b7770d; }
.btn-activate { background: #d5f5e3; color: #145a32; }
.btn-deact    { background: #fdecea; color: #922b21; }

/* ── Status badge pill ────────────────────────────────────────────────────── */
.status-pill { display: inline-flex; align-items: center; gap: 5px; border-radius: 20px; padding: 4px 11px; font-size: 11.5px; font-weight: 600; }
    ...
    <form method="GET" action="admin_staff.php">
        <div class="toolbar mb-4">
            ...
            <?php if($search || $filter_status): ?>
                <a href="admin_staff.php" class="btn btn-filter" style="border:1.5px solid #dee2e6;color:#6c757d;background:#fff;">
                ...
                <tr>
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="fw-semibold" style="font-size:13.5px"><?= htmlspecialchars($st['nama_staff']) ?></div>
                        </div>
                    </td>
                    <td style="font-size:13px"><?= htmlspecialchars($st['jabatan'] ?? '—') ?></td>
                    <td>
                    ...
                    <td style="font-size:13px"><?= htmlspecialchars($st['no_telepon'] ?? '—') ?></td>
                    <td>
...
<!-- ══════════════════ ADD STAFF MODAL ════════════════════════════════════ -->
<div class="modal fade form-modal" id="addStaffModal" tabindex="-1">
            ...
            <form method="POST" action="admin_staff.php" id="addStaffForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex align-items-center justify-content-between p-3"
                        ...
                        <div>
                            <div class="fw-semibold" style="font-size:13px;color:#374151">Active Status</div>
                            <div class="text-muted" style="font-size:11.5px">Staff can receive reservations</div>
                        </div>
...
<!-- ══════════════════ EDIT STAFF MODAL ═══════════════════════════════════ -->
<div class="modal fade form-modal" id="editStaffModal" tabindex="-1">
    <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold">
                    <i class="bi bi-pencil-square me-2 text-warning"></i>Edit Staff
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="detailModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <div class="d-flex align-items-center gap-3">
                    <div id="modal-avatar" class="avatar" style="width:52px;height:52px;font-size:18px;border-radius:14px"></div>
                    <div>
            ...
            <div class="modal-body">
                ...
                <p class="fw-bold text-muted mb-3" style="font-size:11px;text-transform:uppercase;letter-spacing:.8px">Staff Information</p>
                <div id="modal-info"></div>
            </div>
            <div class="modal-footer">
                <a id="modal-schedule-link" href="admin_staffschedule.php" class="btn btn-primary btn-sm"
                ...
                <div class="modal-body pt-2">
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <button type="button" class="btn btn-light fw-semibold"
                                ...
                                data-bs-dismiss="modal" onclick="clearLogoutForm()">
                            Cancel
                        </button>
                    </div>
                </div>
...
function openEditStaff(st) {
    ...
    document.getElementById('edit-jabatan').value = st.jabatan      || '';
    document.getElementById('edit-phone').value   = st.no_telepon   || '';
    document.getElementById('edit-spec').value    = st.id_specialization || '';

    const isActive = parseInt(st.is_active) === 1;
    document.getElementById('edit-active').checked           = isActive;
    document.getElementById('edit-active-label').textContent = isActive ? 'Active' : 'Inactive';
...
function openDetail(st, color, initials) {
    ...
    document.getElementById('ms-spec').textContent = st.nama_specialization || 'General';
    ...
    document.getElementById('modal-schedule-link').href =
        'admin_staffschedule.php?staff=' + st.id_staff;
...
<?php elseif(isset($_GET['edited'])): ?>
    window.addEventListener('DOMContentLoaded', () => showToast('Data staff berhasil diperbarui! ✅'));
<?php endif; ?>
</script>
</body>
</html>